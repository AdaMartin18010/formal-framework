...
**本节要点**：（1）业务逻辑、规则引擎、状态机、工作流的标准化规范；（2）与 L2_D03、formal-model/functional-model 的对应；（3）BPMN、工作流引擎与 L3_D09 的边界。  
**预计阅读时间**：全文约 30–40 分钟；仅读概述 + 核心结构约 12 分钟。  
...
- **对应 L2 元模型**：[L2_D03 功能元模型](L2_D03_功能元模型.md)；L2↔L3 映射总表见 [alignment-L2-L3-matrix](formal-model/alignment-L2-L3-matrix.md)。
- **对应理论**：[formal-model/functional-model/theory.md](formal-model/functional-model/theory.md)
...
- [云原生案例五：Serverless（工作流/状态机）](industry-model/cloud-native-architecture/README.md#案例五serverless-函数计算aws-lambdaknative)
- 金融（交易/风控规则）、Web3（智能合约）、AI（流水线/规则）— 见各 [industry-model](industry-model/) README。

## 8. 权威对标

- **标准与证据**：本模型与 BPMN 2.0、L2_D03 功能元模型对应。参见 [权威对标总索引](reference/AUTHORITY_ALIGNMENT_INDEX.md) 第 2 节。
- **课程映射**：功能/工作流对应 LEARNING_PATHS 初学者阶段1「功能模型」、进阶 MDE；课程见 AUTHORITY_ALIGNMENT_INDEX 第 3 节。
