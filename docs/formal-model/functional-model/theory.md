...
功能建模是业务系统的核心，主流开源项目（如Apache Camel、Spring Statemachine、Drools等）均采用AST或等价结构来描述业务逻辑、规则引擎、状态机、工作流等。其递归结构体现在：

- **业务逻辑节点**：每个业务逻辑为AST的一级节点，包含输入、验证、处理、输出、异常处理等子节点。
- **规则引擎节点**：规则为AST的叶子节点，支持条件、动作、优先级、冲突解决等元数据。
- **状态机节点**：状态、转换、事件、动作等递归嵌套结构。
- **工作流节点**：步骤、依赖、条件、并行、分支等递归组合结构。
- **AST递归遍历**：支持多级嵌套、条件分支、异常处理、并行执行等复杂结构的递归推理与执行。
