...
- **本层细化**：本 L2 元模型为运行时域的基础抽象；具体标准与 DSL 见 L3。
- **下游标准**：[L3_D04 运行时标准模型](L3_D04_运行时标准模型.md)；对象/属性/不变式映射详情见 [L2↔L3 映射总表 2.3 节（运行时）](formal-model/alignment-L2-L3-matrix.md#23-运行时d04)。
...
- **标准与证据**：本模型与 ISO/IEC/IEEE 42010 架构视图、NIST SP 800-190 容器安全、L3_D04 运行时标准对应。参见 [权威对标总索引](reference/AUTHORITY_ALIGNMENT_INDEX.md) 第 2 节、[evidence/STD-42010](evidence/STD-42010.md)。
- **课程映射**：运行时/容器/编排对应 LEARNING_PATHS 云原生专项、L3_D04；课程与认证见 AUTHORITY_ALIGNMENT_INDEX 第 3–4 节（CNCF CKA/CKAD 等）。
