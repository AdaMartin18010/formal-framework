...
**本节要点**：（1）物联网核心对象（设备接入、边缘计算、控制调度、数据采集）与 L3 模型映射；（2）术语对齐与标准/项目映射矩阵；（3）工业协议与开源项目（EdgeX、KubeEdge、OPC UA、LwM2M 等）；（4）技术栈详细映射与成熟案例。  
...
| 本行业（物联网） | L2 元模型 | L3 标准模型 | 说明 |
| ---------------- | --------- | ----------- | ---- |
| 设备接入、协议 | L2_D01 交互、L2_D07 控制调度 | [L3_D01 交互](L3_D01_交互标准模型.md)、[L3_D07 控制调度](L3_D07_控制调度标准模型.md) | MQTT、OPC UA、LwM2M |
| 边缘/网络、编排 | L2_D04 运行时 | [L3_D04 运行时](L3_D04_运行时标准模型.md) | KubeEdge、EdgeX |
| 数据采集、存储 | L2_D02 数据 | [L3_D02 数据](L3_D02_数据标准模型.md) | 时序、IoTDB |
| 监控与告警 | L2_D06 监控 | [L3_D06 监控](L3_D06_监控标准模型.md) | 指标、告警、运行状态 |
