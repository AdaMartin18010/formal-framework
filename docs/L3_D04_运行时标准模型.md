...
**本节要点**：（1）容器化、编排、网络、存储与服务治理的标准化规范；（2）与 L2_D04、L3_D01/L3_D05 的边界；（3）OCI、CNCF、Kubernetes/Istio 等标准与证据。  
**预计阅读时间**：全文约 28–38 分钟；仅读概述 + 核心结构约 12 分钟。  
**单次阅读建议**：建议分 2–4 次阅读，每次 1–2 节，避免单次超过 40 分钟；结合 [REVIEW_CHECKLIST](learning/REVIEW_CHECKLIST.md) 做阶段自测。

## 1. 概述

运行时标准模型定义了系统运行时的标准化规范，包括容器化、编排、网络、存储和服务治理。
该模型基于Kubernetes、Docker、Istio等云原生技术栈，以及现代微服务架构的最佳实践。

### 1.1 前置与关联

- **对应 L2 元模型**：[L2_D04 运行时元模型](L2_D04_运行时元模型.md)；对象/属性/不变式映射详情见 [L2↔L3 映射总表 2.3 节（运行时）](formal-model/alignment-L2-L3-matrix.md#23-运行时d04)。
- **对应理论**：[formal-model/runtime-model/theory.md](formal-model/runtime-model/theory.md)
- **与 L3 边界**：本模型聚焦**容器、编排、工作负载与运行时资源**；API 契约与协议设计见 [L3_D01 交互标准模型](L3_D01_交互标准模型.md)；部署配置与发布见 [L3_D05 部署标准模型](L3_D05_部署标准模型.md)。
...
- [云原生案例一：Kubernetes 集群编排](industry-model/cloud-native-architecture/README.md#案例一kubernetes-集群编排基础)
- [云原生案例二：Service Mesh 流量治理（Istio）](industry-model/cloud-native-architecture/README.md#案例二service-mesh-流量治理istio)
- [云原生案例五：Serverless 函数计算](industry-model/cloud-native-architecture/README.md#案例五serverless-函数计算aws-lambdaknative)
- [云原生案例六：API 网关流量治理（Kong/Envoy）](industry-model/cloud-native-architecture/README.md#案例六api-网关流量治理kongenvoy)
- 其他行业：AI 基础设施、物联网、金融、Web3 — 见各 [industry-model](industry-model/) 目录下 README。

## 8. 权威对标

- **标准与证据**：本模型与 ISO/IEC/IEEE 42010、NIST SP 800-190 容器安全、L2_D04 运行时元模型对应。参见 [权威对标总索引](reference/AUTHORITY_ALIGNMENT_INDEX.md) 第 2 节、[evidence/STD-42010](evidence/STD-42010.md)；CNCF 见第 4 节。
- **课程映射**：运行时/编排对应 LEARNING_PATHS 云原生专项；认证见 CKA、CKAD、ICA 等。
