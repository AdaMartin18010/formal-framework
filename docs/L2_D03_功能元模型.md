...
**本节要点**：（1）功能域基础抽象：业务逻辑、规则引擎、状态机、工作流；（2）与 L3_D03、formal-model/functional-model 的对应；（3）演进策略与质量保证。  
**预计阅读时间**：全文约 28–38 分钟；仅读概述 + 元模型元素约 12 分钟。
...
- **本层细化**：本 L2 元模型为功能域的基础抽象；具体标准与 DSL 见 L3。
- **下游标准**：[L3_D03 功能标准模型](L3_D03_功能标准模型.md)；L2↔L3 映射总表见 [alignment-L2-L3-matrix](formal-model/alignment-L2-L3-matrix.md)（功能域与 L3_D03 对应）。
...
- **标准与证据**：本模型与 BPMN 2.0 业务流程、L3_D03 功能标准模型对应。参见 [权威对标总索引](reference/AUTHORITY_ALIGNMENT_INDEX.md) 第 2 节。
- **课程映射**：功能/工作流/状态机对应 LEARNING_PATHS 初学者阶段1「功能模型」、进阶「模型驱动工程」；课程见 AUTHORITY_ALIGNMENT_INDEX 第 3 节。
