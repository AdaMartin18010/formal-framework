...
**本节要点**：（1）实体设计、字段定义、关系建模、索引与数据迁移的标准化规范；（2）与 L2_D02、formal-model/data-model 的对应；（3）关系型与 NoSQL、数据完整性不变式。  
...
- **对应 L2 元模型**：[L2_D02 数据元模型](L2_D02_数据元模型.md)；对象/属性/不变式映射详情见 [L2↔L3 映射总表 2.2 节（数据）](formal-model/alignment-L2-L3-matrix.md#22-数据d02)。
...
- **ISO 11179**：数据元素注册标准
- **ISO/IEC 27001**：信息安全管理，见 [evidence/STD-27001](evidence/STD-27001.md)
...
- 云原生（配置/Secret）、金融（账户/交易/报表）、IoT（时序/设备数据）、AI（特征存储、训练数据）、Web3（链上状态）— 见各 [industry-model](industry-model/) README 与 L4 行业索引。

## 8. 权威对标

- **标准与证据**：本模型与 ISO/IEC 25012 数据质量、L2_D02 数据元模型对应。参见 [权威对标总索引](reference/AUTHORITY_ALIGNMENT_INDEX.md) 第 2 节、[evidence/STD-15288](evidence/STD-15288.md)。
- **课程映射**：数据标准对应 LEARNING_PATHS 初学者阶段1「数据模型」；课程见 AUTHORITY_ALIGNMENT_INDEX 第 3 节。
