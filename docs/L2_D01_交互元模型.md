...
**本节要点**：（1）交互域基础抽象：Endpoint、Resource、Message、Protocol、Contract；（2）与 L3_D01、formal-model/interaction-model 的对应；（3）形式化不变式与验证规则。  
**预计阅读时间**：全文约 25–35 分钟；仅读概述 + 元模型元素约 12 分钟。

## 1. 概述

- **范围**：API、协议、消息、[契约](formal-model/interaction-model/theory.md)、安全、性能、监控
- **目标**：定义[交互模型](knowledge-standards/concept-index/CONCEPT_INDEX.md)域的基础抽象、约束和验证规则
- **原则**：可验证性、一致性、可追溯性、安全性、性能优化
- **核心术语**（[形式化建模](formal-model/core-concepts/formal-modeling.md)、[元模型](knowledge-standards/glossary/GLOSSARY.md)、不变式等）首次出现处可参见 [术语表](knowledge-standards/glossary/GLOSSARY.md) 与 [概念索引](knowledge-standards/concept-index/CONCEPT_INDEX.md)。

### 1.1 前置与关联

- **对应理论**：[formal-model/interaction-model/theory.md](formal-model/interaction-model/theory.md)
- **本层细化**：本 L2 元模型为交互域的基础抽象；具体标准与 DSL 见 L3。
- **下游标准**：[L3_D01 交互标准模型](L3_D01_交互标准模型.md)；对象/属性/不变式映射详情见 [L2↔L3 映射总表 2.1 节（交互）](formal-model/alignment-L2-L3-matrix.md#21-交互d01)。

**阅读顺序建议**：

| 建议先读（前置） | 建议后读（后续） |
|------------------|------------------|
| [形式化建模](formal-model/core-concepts/formal-modeling.md)、[README 模型体系](README.md#4-各模型关系与递归扩展路径) | [L3_D01 交互标准模型](L3_D01_交互标准模型.md)、[interaction-model/theory](formal-model/interaction-model/theory.md) |
...
- **标准与证据**：本模型与 ISO/IEC/IEEE 42010 架构视图、OpenAPI 契约描述对应；验证与确认可对齐 IEEE 1012。参见 [权威对标总索引](reference/AUTHORITY_ALIGNMENT_INDEX.md) 第 2 节、[evidence/STD-42010](evidence/STD-42010.md)、[evidence/STD-OAS31](evidence/STD-OAS31.md)。
- **课程映射**：交互/API/契约对应 LEARNING_PATHS 初学者阶段1「交互模型」、进阶「DSL 与类型」；课程见 AUTHORITY_ALIGNMENT_INDEX 第 3 节（如 Stanford CS 242、MIT 6.035）。
