...
**规范文档入口**：贡献与评审须对齐以下文档（详见各文档内容）：
- [引用规范 (CITATION_STANDARDS.md)](CITATION_STANDARDS.md) — 关键结论须具备高可信引用
- [质量规范 (QUALITY_STANDARDS.md)](QUALITY_STANDARDS.md) — 完成度等级、结构、图表、链接
- [专家评审体系 (EXPERT_REVIEW_SYSTEM.md)](EXPERT_REVIEW_SYSTEM.md) — RC/FINAL 评审流程与记录
- [代码审查指南 (CODE_REVIEW_GUIDE.md)](CODE_REVIEW_GUIDE.md) — 代码与配置变更审查
