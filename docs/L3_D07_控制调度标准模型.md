...
**本节要点**：（1）任务调度、实时约束、事件驱动、状态机四类核心能力；（2）与 L2_D07 及 IEEE/IEC/ISO 标准的对齐；（3）调度算法与不变式（截止期、可调度性）；（4）行业映射（IoT、工业、云原生调度）。  
**预计阅读时间**：全文约 25–35 分钟；仅读 §1–§2 约 10 分钟。  
**单次阅读建议**：建议分 2–4 次阅读，每次 1–2 节（约 15–20 分钟），结合 [REVIEW_CHECKLIST](learning/REVIEW_CHECKLIST.md) 做阶段自测。
...
- **对应 L2 元模型**：[L2_D07 控制调度元模型](L2_D07_控制调度元模型.md)；对象/属性/不变式映射详情见 [L2↔L3 映射总表 2.6 节（控制调度）](formal-model/alignment-L2-L3-matrix.md#26-控制调度d07)。
- **对应理论**：[formal-model/](formal-model/) 中与控制调度相关的建模
...
**本段检查点**：此处可暂停；建议先能说出 L2_D07 与 L3_D07 的对应关系及四类核心能力（任务调度、实时约束、事件驱动、状态机），再继续 §2。自测可参考 [LEARNING_PATHS 阶段2](LEARNING_PATHS.md) 与 [REVIEW_CHECKLIST](learning/REVIEW_CHECKLIST.md)。
...
- [云原生案例一：Kubernetes（调度器/资源管理）](industry-model/cloud-native-architecture/README.md#案例一kubernetes-集群编排基础)
- IoT（边缘调度、设备任务）、AI（训练/推理调度）、金融（交易调度）— 见各 [industry-model](industry-model/) README。

## 8. 权威对标

- **标准与证据**：本模型与 L2_D07 控制调度元模型对应；实时与调度可参考 ISO 26262、IEC 61131。参见 [权威对标总索引](reference/AUTHORITY_ALIGNMENT_INDEX.md) 第 2 节。
- **课程映射**：控制调度对应 LEARNING_PATHS 物联网专项、L4_D92；行业见 industry-model/iot-architecture。
