        ...
        for i, file_path in enumerate(markdown_files, 1):
            ...
            try:
                ...
                if result['broken_links']:
                    print(f"❌ {file_path.name}: {result['valid_links']}/{result['total_links']} 链接有效 ({len(result['broken_links'])} 个无效)")
                else:
...
def main():
    ...
    validator = SimpleLinkValidator(args.root)
